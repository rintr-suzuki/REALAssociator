 ...
 *  Feb.     2021			R. Suzuki	(change for sedimentary correction) //RS
 *  Apr.     2021			R. Suzuki	(change for magnitude calculation) //RS2
 *  May      2021			R. Suzuki	(bug fix for sorting) //RS3
 *  Aug.     2021			R. Suzuki	(visualization of excluded events) //RS4
 *  Oct.     2021			R. Suzuki	(optimization of sorting associated picks) //RS5
 *  Feb.     2024			R. Suzuki	(add output dir arg) //RS6
 ************************************************************************/
 //pscounts[nnn-1] -> pscount_best //RS5
...
//time_t timer0, timer1, timer2, timer3, timer4, timer5; //RS5
...
void Accounttriggers_homo(double,double,double,double,double,double,double,int); //RS
void Accounttriggers_layer(double,double,double,double,double,double,double,int); //RS
//void Sortpscounts(double **,int); //RS5
void Getbestpscounts(double **,double *,int); //RS5


//global 
double tint;
double **pscounts;
double *pscount_best; //RS5
STATION *ST;
TRIG **TGP,**TGS,**TGP0,**TGS0; //RS3
TTT *TB;
double ptw,stw,nrt;
...
int Nst = 1500; //maximum number of stations //RS6
int Nps = 20000; //maximum number of P/S picks recorded at one station
int Ntb = 130000; //maximum number of lines in traveltime table //RS5
int ispeed = 1; //default setting ispeed = 1

char PHASESEL[256],CATALOGSEL[256],RESOLUTION[256]; //RS4

//main function
int main(int argc, char **argv){
	int i,j,k,l,m,n;
	FILE *fp,*fpr,*fp1,*fp2;
	char output1[256],output2[256],dir[256],input[256],outdir[256]; //RS6
    ...
    if(argc < 3 || error == 1){
        ...
        fprintf(stderr, "   -S(np0/ns0/nps0/nppp/std0/dtps/nrt[/rsel/ires]) [-G(trx/trh/tdx/tdh)] station pickdir ttime outdir\n"); //RS6
	...
	sprintf(ftail,"%04d%02d%02d",nyear,nmon,nday); //RS4

    /* make output files */ //RS6
	if(igrid == 0){strcpy(outdir,argv[8]);}else{strcpy(outdir,argv[9]);}

	sprintf(PHASESEL,"%s/phase_sel.txt",outdir); //RS4,6 
	sprintf(CATALOGSEL,"%s/catalog_sel.txt",outdir); //RS4,6
	sprintf(RESOLUTION,"%s/resolution.txt",outdir); //RS4,6
	...
	if(ielev == 0){
		...
		for(i=0;i<Nst;i++)ST[i].stcorrp = 0.0; //RS
		for(i=0;i<Nst;i++)ST[i].stcorrs = 0.0; //RS
	} else if (ielev == 1) { //RS
		for(i=0;i<Nst;i++)ST[i].stcorrp = 0.0; //RS
		for(i=0;i<Nst;i++)ST[i].stcorrs = 0.0; //RS
	} else if (ielev == 2) { //RS
		for(i=0;i<Nst;i++)ST[i].elev = 0.0; //RS
	}
	...
	//RS3**
	TGP0= (TRIG **)malloc(sizeof(TRIG*)*Nst);
	TGS0 = (TRIG **)malloc(sizeof(TRIG*)*Nst);
	for(i=0;i<Nst;i++){
		TGP0[i] = (TRIG *)malloc(sizeof(TRIG)*Nps);
		TGS0[i] = (TRIG *)malloc(sizeof(TRIG)*Nps);
	}
	//RS3**
	...
	for(i=0;i<Nst;i++){
        ...
        if((fp=fopen(input,"r"))==NULL){
			...
			for (j = 0; j < Nps; j++){
				...
				//fprintf(stderr,"ppick0:%lf pamp0:%lf pweight0: %lf\n",TGP[i][j].trig,TGP[i][j].amp,TGP[i][j].weight);
				if(test==1)break;
			}
		  	fclose(fp);
		}
        ...
        if((fp=fopen(input,"r"))==NULL){
			...
			for (j = 0; j < Nps; j++){
				...
				//fprintf(stderr,"spick0:%lf samp0:%lf sweight0: %lf\n",TGS[i][j].trig,TGS[i][j].amp,TGS[i][j].weight);
				if(test==1)break;
			}
		  	fclose(fp);
		}
	...
	for(i=0;i<Nst;i++){
		for(j=0;j<Nps;j++){
			TGP0[i][j].trig = TGP[i][j].trig;
			TGP0[i][j].weight = TGP[i][j].weight;
			TGP0[i][j].amp = TGP[i][j].amp;
			TGS0[i][j].trig = TGS[i][j].trig;
			TGS0[i][j].weight = TGS[i][j].weight;
			TGS0[i][j].amp = TGS[i][j].amp;
		}
	}
	//RS3**
   ...
   SortTriggers(TGP0,TGS0,ptrig,strig,Nst,Nps); //RS3
	//sort triggers
   ...
   printf("Nlat= %d Nlon= %d Ndep= %d\n",nlat,nlon,ndep);
	...
	pscount_best = (double *)malloc(9*sizeof(double)); //RS5

	//timer1 = time(NULL); //RS5
  	//printf("readphase %f\n", difftime(timer0,timer1)); //RS5
	...
	if(latref0 < -999 && lonref0 < -999)inoref = 1;
    	Maxt0 = Find_max(ptrig,Nst,Nps); //全観測点,イベントで一番早い時刻を探す
	//search each initiating P pick
	while(Find_min(ptrig,Nst,Nps) < Maxt0){
		//jump:
		Nps = DetermineNp(ptrig,Nst,Nps);
		Find_min_loc(ptrig,Nst,1,&tpmin0,&m,&n); //各観測点で一番早い時刻(tpmin0)を探す
		if(fabs(tpmin0 - 1.0e8) < 1) break;

		lonref = ST[m].stlo;
		latref = ST[m].stla;
		elevref = ST[m].elev;
		corrpref = ST[m].stcorrp; //RS
		...
		//homo model
		if(igrid == 0){
			#pragma omp parallel for shared(pscounts) firstprivate(latref,lonref,latref0,lonref0,elevref,corrpref,nlon,ndep,dx,dh) private(lat0,lon0,dep,l,i,j,k) //RS
			for(l=0; l<nnn; ++l){
				i = (int)(l/(nlon*ndep));
				...
				dep =  k*dh;
				Accounttriggers_homo(lat0,lon0,dep,latref,lonref,elevref,corrpref,l); //RS
			}
			#pragma omp barrier
		//layer model
		}else{
			...
			for(l=0; l<nnn; ++l){
				...
				lon0 = lonref0 - rx + j*dx + 0.01234*dx;
				dep =  k*dh;
				Accounttriggers_layer(lat0,lon0,dep,latref,lonref,elevref,corrpref,l); //RS
			}
			#pragma omp barrier
		}

		//timer2 = time(NULL); //RS5
		//printf("asoc-parallel %f\n", difftime(timer0,timer2)); //RS5

		//sort pscounts
		//*RS5*
		//各仮震源に対してのアソシエーション状況をpick数順にソートする
		//一番よいやつ[nnn-1]しかこの後使っていないが、c言語では最大値の計算にも自力でソートするしかないらしい
		//最大値を得るだけであれば比較的簡単にできそうなので、並列化してみる
		//特にassociateされたpick数が少ない場合、並列計算で順番が混ざるので、同じstd, psを持つ、REAL_v4.cとは異なった出力になる場合がある
		//cf. https://annex.jsap.or.jp/photonics/kogaku/public/39-10-kougakukoubou.pdf
		//cf. https://www.cc.u-tokyo.ac.jp/public/VOL9/special/9.pdf

		//pscount_tmpbest = (double *)malloc(9*sizeof(double));
		#pragma omp parallel
		{
			double pscount_tmpbest[9];
			for(l=0;l<9;l++){
				pscount_tmpbest[l] = 0.0;
			}
			pscount_tmpbest[6] = 1.0e8;
		#pragma omp for
			for(i=0;i<nnn;i++){
				Getbestpscounts(pscounts,pscount_tmpbest,i); 
			}
		#pragma omp critical
			{
				if (pscount_tmpbest[7] > pscount_best[7] || (pscount_tmpbest[7] == pscount_best[7] && pscount_tmpbest[6] < pscount_best[6])){
					//[6] std: std of time residuals between calculated and picked
					//[7] ps: the number of P + S associated pick

					// replace best with tmpbest if tmpbest is better
					for(k=0;k<9;k++){
						pscount_best[k] = pscount_tmpbest[k];
						}
					//told = pscount_tmpbest[3];
					//ttd = (int)(pscount_tmpbest[3]/86400);
					//tth = (int)((pscount_tmpbest[3] - ttd*86400)/3600);
					//tts = (int)((pscount_tmpbest[3] - ttd*86400 - tth*3600)/60);
					//ttm = pscount_tmpbest[3] - ttd*86400 - tth*3600 - tts*60;
					//sprintf(otime,"%04d %02d %02d %02d:%02d:%06.3f",nyear,nmon,ttd+nday,tth,tts,ttm);
					//fprintf(stderr,"-tmp- %5d %25s %12.3lf %8.4lf %12.4lf %12.4lf %12.4lf %4d %4d %4d %8.2f\n",otime,pscount_tmpbest[3],pscount_tmpbest[6],pscount_tmpbest[0],pscount_tmpbest[1],pscount_tmpbest[2],(int)(pscount_tmpbest[4]),(int)(pscount_tmpbest[5]),(int)(pscount_tmpbest[7]),pscount_tmpbest[8]);
				}
			}
		}
		//*RS5*
		//Sortpscounts(pscounts,nnn); //RS5

		//timer5 = time(NULL); //RS5
		//printf("sortpick %f\n", difftime(timer0,timer5)); //RS5
        
		if(ppp >= nppp && pscount_best[7] >= nps0  && pscount_best[6] <= std0 && pscount_best[8] <= GAPTH){
        	//remove events out of the station region
		//if(ppp >= nppp && pscount_best[7] >= nps0  && pscount_best[6] <= std0 && pscount_best[8] <= GAPTH && pscount_best[0] >= stlamin && pscount_best[0] <= stlamax && pscount_best[1] >= stlomin && pscount_best[1] <= stlomax){
       	 	//remove events out of the station region and at the depth of h == rh (insufficient depth search)
        	//if(ppp >= nppp && pscount_best[7] >= nps0  && pscount_best[6] <= std0 && pscount_best[8] <= GAPTH && pscount_best[0] >= stlamin && pscount_best[0] <= stlamax && pscount_best[1] >= stlomin && pscount_best[1] <= stlomax && pscount_best[2] < rh){
			told = pscount_best[3];
			ttd = (int)(pscount_best[3]/86400);
			tth = (int)((pscount_best[3] - ttd*86400)/3600);
			tts = (int)((pscount_best[3] - ttd*86400 - tth*3600)/60);
			ttm = pscount_best[3] - ttd*86400 - tth*3600 - tts*60;
			sprintf(otime,"%04d %02d %02d %02d:%02d:%06.3f",nyear,nmon,ttd+nday,tth,tts,ttm);

			RELC[mmm].num1 = mmm+1;
			strcpy(RELC[mmm].otime1,otime);
			RELC[mmm].atime1 = pscount_best[3];
			RELC[mmm].std1 = pscount_best[6];
			RELC[mmm].lat1 = pscount_best[0];
			RELC[mmm].lon1 = pscount_best[1];
			RELC[mmm].dep1 = pscount_best[2];
			RELC[mmm].nofp1 = pscount_best[4];
			RELC[mmm].nofs1 = pscount_best[5];
			RELC[mmm].ntotal1 = pscount_best[7];

         fprintf(stderr,"%5d %25s %12.3lf %8.4lf %12.4lf %12.4lf %12.4lf %4d %4d %4d %8.2f\n",mmm+1,otime,pscount_best[3],pscount_best[6],pscount_best[0],pscount_best[1],pscount_best[2],(int)(pscount_best[4]),(int)(pscount_best[5]),(int)(pscount_best[7]),pscount_best[8]);

			//*RS4*
			//only output the resolution file for the first effective event (the first pick should be true) -> not first because of the parallel calculation
			if(ires == 1){
				//fpr = fopen(RESOLUTION,"w");
				fpr = fopen(RESOLUTION,"a"); //RS4
				fprintf(fpr,"%5d %25s %12.3lf %8.4lf %12.4lf %12.4lf %12.4lf %4d %4d %4d %8.2f\n",mmm+1,otime,pscount_best[3],pscount_best[6],pscount_best[0],pscount_best[1],pscount_best[2],(int)(pscount_best[4]),(int)(pscount_best[5]),(int)(pscount_best[7]),pscount_best[8]);
				fclose(fpr);
				}
				//exit(-1);
			//*RS4*
			...
			if(ispeed > 1.0e-5){
				for(k=0;k<Nst;k++){
					//仮震源から各観測点までの理論走時を計算して、窓の中に入った"P"pickを全て捨てる
					lat0 = pscount_best[0];
					lon0 = pscount_best[1];
					dep  = pscount_best[2];
					...
					if(igrid == 0){
						tp_cal = sqrt((GCarc*111.19)*(GCarc*111.19) + dep*dep)/vp0 + ST[k].elev/s_vp0 + ST[k].stcorrp; //RS
					}else{
						ih = rint(dep/tdh);
						ig = ih*rint(trx/tdx) + rint(GCarc/tdx);
						tp_cal = TB[ig].ptime + (GCarc - TB[ig].gdist)*TB[ig].prayp + (dep - TB[ig].dep)*TB[ig].phslow + ST[k].elev/s_vp0 + ST[k].stcorrp; //RS
					}

					tp_pre = pscount_best[3] + tp_cal; //pscount_best[3]: 仮の発震時刻
		...
		fprintf(stderr,"-exc- %25s %12.3lf %8.4lf %12.4lf %12.4lf %12.4lf %4d %4d %4d %8.2f\n",otime,pscount_best[3],pscount_best[6],pscount_best[0],pscount_best[1],pscount_best[2],(int)(pscount_best[4]),(int)(pscount_best[5]),(int)(pscount_best[7]),pscount_best[8]);

		//only output the resolution file for the first effective event (the first pick should be true)
		if(ires == 1){
			//fpr = fopen(RESOLUTION,"w");
			fpr = fopen(RESOLUTION,"a"); //RS4
			fprintf(fpr,"-exc- %25s %12.3lf %8.4lf %12.4lf %12.4lf %12.4lf %4d %4d %4d %8.2f\n",otime,pscount_best[3],pscount_best[6],pscount_best[0],pscount_best[1],pscount_best[2],(int)(pscount_best[4]),(int)(pscount_best[5]),(int)(pscount_best[7]),pscount_best[8]);
			fclose(fpr);
			}
			//exit(-1);
		//*RS4*
		}
		//timer3 = time(NULL); //RS5
		//printf("firstselection %f\n", difftime(timer0,timer3)); //RS5
	}
	...
	for(i=0;i<nselect;i++){
		...
		for(k=0;k<Nst;k++){
         ...
         rdist = sqrt((GCarc*111.19)*(GCarc*111.19) + dep*dep);
			if(igrid == 0){
				tp_cal = rdist/vp0 + ST[k].elev/s_vp0 + ST[k].stcorrp;
				ts_cal = rdist/vs0 + ST[k].elev/s_vs0 + ST[k].stcorrs;
			}else{
				ih = rint(dep/tdh);
				ig = ih*rint(trx/tdx) + rint(GCarc/tdx);
				tp_cal = TB[ig].ptime + (GCarc - TB[ig].gdist)*TB[ig].prayp + (dep - TB[ig].dep)*TB[ig].phslow + ST[k].elev/s_vp0 + ST[k].stcorrp;
				ts_cal = TB[ig].stime + (GCarc - TB[ig].gdist)*TB[ig].srayp + (dep - TB[ig].dep)*TB[ig].shslow + ST[k].elev/s_vs0 + ST[k].stcorrs;
			}
			...
			for(j=0;j<NNps;j++){
				...
				if(ptrig0[k][j] > tp_pre_b && ptrig0[k][j] < tp_pre_e && fabs(ptrig0[k][j]-tp_pre) < rsel*RELC[i].std1 && GCarc<GCarc0){
					...
					//p_mag = log(pamp0[k][j])/log(10) + 1.110*log(rdist/100)/log(10) + 0.00189*(rdist-100)+3.0; //RS2
					p_mag = (log10(pamp0[k][j]*100) + 1.73*log10(rdist)+2.5)/0.85; //RS2
					//fprintf(stderr,"pamp:%lf p_mag: %lf\n",pamp0[k][j],p_mag);
			...
			for(j=0;j<NNps;j++){
				if((ts_pre-tp_pre) > dtps && fabs(ptemp - strig0[k][j]) > dtps && strig0[k][j] > ts_pre_b && strig0[k][j] < ts_pre_e && fabs(strig0[k][j]-ts_pre) < rsel*RELC[i].std1 && GCarc < GCarc0){
					...
					//s_mag = log(samp0[k][j])/log(10) + 1.110*log(rdist/100)/log(10) + 0.00189*(rdist-100)+3.0; //RS2
					s_mag = (log10(samp0[k][j]*100) + 1.73*log10(rdist)+2.5)/0.85; //RS2
					//fprintf(stderr,"samp:%lf s_mag: %lf\n",samp0[k][j],s_mag);
	...
	//timer4 = time(NULL); //RS5
	//printf("secondselection %f\n", difftime(timer3,timer4)); //RS5
}
...
int ReselectClear(CLEARUP *CLEAR,int NN){
    ...
    for(i=0;i<NN;i++){
        //select again (0.8*nps0)
		  //fprintf(stderr,"np0: %d %d\n ns0: %d %d\n nps0: %d %d\n",np0,CLEAR[i].pcount,ns0,CLEAR[i].scount,nps0,CLEAR[i].pscount); //RSSSS
...
int Readstation(char *name, STATION *ST, int nmax){
        ...
        for (i = 0; i <= nmax; i++){
            if(fscanf(infile, "%lf %lf %s %s %s %lf %lf %lf\n",&ST[i].stlo,&ST[i].stla,ST[i].net,ST[i].sta,ST[i].comp,&ST[i].elev,&ST[i].stcorrp,&ST[i].stcorrs)==EOF)test=1; //RS
            if(test==1)break;
        }
        fclose(infile);
        return i;
...
void SortTriggers(TRIG **tgp,TRIG **tgs,double **array1,double **array2,int m, int n){
	...
	for (i = 0; i < m; ++i){
		for (j = 0; j < n; ++j){
			//fprintf(stderr,"ppick0_0:%lf pamp0_:%lf pweight0_: %lf\n",tgp[i][j].trig,tgp[i][j].amp,tgp[i][j].weight);
			for (k =(j + 1); k < n; ++k){
			...
			//fprintf(stderr,"ppick0_1:%lf pamp0_:%lf pweight0_: %lf\n",tgp[i][j].trig,tgp[i][j].amp,tgp[i][j].weight);
        }
    }
...
void SortTriggers0(TRIG **tgp,TRIG **tgs,double **array1,double **array2,double **pamp,double **samp,double **pweight,double **sweight,int m, int n){
	...
	//trig(pickの時刻)に関するソート(SortTriggersと同じ)
	for (i = 0; i < m; ++i){
		for (j = 0; j < n; ++j){
			//各pickのそれ以降のpickに関してループを回す
			//fprintf(stderr,"ppick0_:%lf pamp0_:%lf pweight0_: %lf\n",tgp[i][j].trig,tgp[i][j].amp,tgp[i][j].weight);
			for (k =(j + 1); k < n; ++k){
				//pに関して、後ろのpickに早いpickがあったら行を入れ替える
				if (tgp[i][j].trig > tgp[i][k].trig){
					a = tgp[i][j].trig;
					b = tgp[i][j].weight;
					c = tgp[i][j].amp;
				...
				//sに関して、後ろのpickに早いpickがあったら行を入れ替える
				if (tgs[i][j].trig > tgs[i][k].trig){
					a = tgs[i][j].trig;
					b = tgs[i][j].weight;
					c = tgs[i][j].amp;
			...
			//fprintf(stderr,"ppick1:%lf pamp1:%lf pweight1: %lf\n",tgp[i][j].trig,tgp[i][j].amp,tgp[i][j].weight);
        }
    }
	
	//重み(weight)を用いたpickの選別(SortTriggersと同じ)
	for(i=0;i<m;i++){
		array1[i][0] = tgp[i][0].trig;
		array2[i][0] = tgs[i][0].trig;
      ...
      sweight[i][0] = tgs[i][0].weight;
		for(j=1;j<n;j++){
			//pに関して、隣り合うpickの時刻が近い場合、
			if(tgp[i][j].trig - tgp[i][j-1].trig < nrt*ptw){
				//確率(weight)が小さい方のpickは消す
			   if(tgp[i][j].weight > tgp[i][j-1].weight){
					array1[i][j] = tgp[i][j].trig;
			...
			//fprintf(stderr,"ppick2:%lf pamp2:%lf pweight2: %lf\n",array1[i][j],pamp[i][j],pweight[i][j]);
		}
	}
	
	//(2回目)trig(pickの時刻)に関するソート(SortTriggersと同じ)
	for (i = 0; i < m; ++i){
        for (j = 0; j < n; ++j){
			  //各pickのそれ以降のpickに関してループを回す
            for (k =(j + 1); k < n; ++k){
				...
				//fprintf(stderr,"ppick3:%lf pamp3:%lf pweight3: %lf\n",array1[i][j],pamp[i][j],pweight[i][j]);
				//fprintf(stderr,"spick:%lf samp:%lf sweight: %lf\n",array2[i][j],samp[i][j],sweight[i][j]);
        }
    }
}
...
void Sortpscounts(double **pscounts0,int np){
	...
	//i: former, j: latter
	//see each i and search worse one after i (j)
	//finally, the last one is the best group of picks 
...
//*RS5*
void Getbestpscounts(double **pscounts0,double *tmpbest,int i){
	int k;

	if (pscounts0[i][7] > tmpbest[7] || (pscounts0[i][7] == tmpbest[7] && pscounts0[i][6] < tmpbest[6])){
	//[6] std: std of time residuals between calculated and picked
	//[7] ps: the number of P + S associated pick

	// replace tmpbest with i if i is better
	for(k=0;k<9;k++){
		tmpbest[k] = pscounts0[i][k];
	}	
		}
}
//*RS5*

void Accounttriggers_homo(double lat0,double lon0,double dep,double latref,double lonref,double elevref,double corrpref, int l){ //RS
	...
	tp0_cal = sqrt((GCarc*111.19)*(GCarc*111.19) + dep*dep)/vp0 + elevref/s_vp0 + corrpref; //RS


	for(i=0;i<Nst;i++){
		ddistaz(ST[i].stla,ST[i].stlo,lat0,lon0,&GCarc,&baz);
		tp_cal = sqrt((GCarc*111.19)*(GCarc*111.19) + dep*dep)/vp0 + ST[i].elev/s_vp0 + ST[i].stcorrp; //RS
		ts_cal = sqrt((GCarc*111.19)*(GCarc*111.19) + dep*dep)/vs0 + ST[i].elev/s_vs0 + ST[i].stcorrs; //RS
...
void Accounttriggers_layer(double lat0,double lon0,double dep,double latref,double lonref,double elevref,double corrpref, int l){ //RS
	//各仮震源グリッドごとの計算
	int pcount,scount,ps;
	int i,j,k,ig,ih;
	double GCarc,baz,median,std,ptemp;
   ...
   extern double GCarc0,std0;
	...
	ih = round(dep/tdh);
	ig = ih*rint(trx/tdx) + rint(GCarc/tdx);
	tp0_cal = TB[ig].ptime + (GCarc - TB[ig].gdist)*TB[ig].prayp + (dep - TB[ig].dep)*TB[ig].phslow + elevref/s_vp0 + corrpref; //RS
	
	for(i=0;i<Nst;i++){
		//観測点の位置と理論(相対)走時
		ddistaz(ST[i].stla,ST[i].stlo,lat0,lon0,&GCarc,&baz); //各観測点ごとの大円距離(GCarc)と射出角度(baz)を計算し直している
		ih = rint(dep/tdh);
		ig = ih*rint(trx/tdx) + rint(GCarc/tdx);
		tp_cal = TB[ig].ptime + (GCarc - TB[ig].gdist)*TB[ig].prayp + (dep - TB[ig].dep)*TB[ig].phslow + ST[i].elev/s_vp0 + ST[i].stcorrp; //RS
		ts_cal = TB[ig].stime + (GCarc - TB[ig].gdist)*TB[ig].srayp + (dep - TB[ig].dep)*TB[ig].shslow + ST[i].elev/s_vs0 + ST[i].stcorrs; //RS
						
		tp_pre = tpmin0 - tp0_cal + tp_cal; //走時表は相対値なので、最小のP波到達時刻=絶対時刻の基準値(tpmin0)+観測点走時(tp_cal)-震源走時(tp0_cal)としている
		ts_pre = tpmin0 - tp0_cal + ts_cal;
		...
		for(j=np0_start[i];j<np0_end[i];j++){
			if(ptrig0[i][j] > tp_pre_b && ptrig0[i][j] < tp_pre_e && GCarc < GCarc0){
				//printf("%lf < %lf < %lf\n",tp_pre_b,ptrig0[i][j],tp_pre_e);
				torg[ps] = ptrig0[i][j] - tp_cal; //ptrig0[i][j]は上記tp_preに当たる、つまり、torg[ps]~tpmin0 - tp0_calは発震時刻を示す
                stagap[ps] = baz;
				pcount = pcount + 1; ps = ps + 1;
				ptemp = ptrig0[i][j];
				break;
